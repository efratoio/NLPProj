...
def create_network(config,word_index):
	...
	if config["VANILA"]:
		...
		if config["PROPS"]:
			...
			last_layer = Dense(config["RNN_DIM"]*2)(auxiliary_input)
			last_layer = Bidirectional(LSTM(config["RNN_DIM"],return_sequences=config["TURN_ATTENTION"]))(concatenate([chats_encoder, last_layer]))
		else:
