...
\begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=\layersep]
    ...
    \node[output neuron, right of=A-turn-3] (V) at (\layersept + \layersep+\layersep+\layersep,-2.2)  {$\vec{v_d}$};
    % \node[output neuron, right of=V] (sm) {softmax};

    \node[output neuron, above of=H-rev-1] (U) {$\vec{u_w}$};
    \node[output neuron, above of=H-rev-sent-1] (Us) {$\vec{u_s}$};
    \node[output neuron, above of=H-rev-turn-1] (Ut) {$\vec{u_t}$};
    \node[semantic neuron] (sem) at (\layersept + \layersep+\layersep+\layersep,-0.2) {$\vec{u_{sem}}$};

   
    % \node[output neuron, right of=H-3] (O) {};
    % Draw the output layer node
    \node[text centered, pin={[pin edge={->}]right:Output}] (O) at (\layersept + \layerseps + \layersep,-2.2) {softmax};
    ...
    \foreach \source in {1,...,4}{
        \path (H-rev-\source) edge (A-\source);
        \path (U) edge (A-\source);

        \path (H-rev-sent-\source) edge (A-sent-\source);
        \path (Us) edge (A-sent-\source);

        \path (H-rev-turn-\source) edge (A-turn-\source);
